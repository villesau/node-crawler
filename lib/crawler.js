...
class Crawler extends EventEmitter {
  ...
  init(options) {
    ...
    this.limiters = new Bottleneck({
      maxConcurrent: this.options.maxConnections,
    ...
    this.seen
      ...
      .catch((e) => log('error', e));
    this.limiters.on('failed', async (error, jobInfo) => {
      if (
        error instanceof CrawlError &&
        error.options.retries > jobInfo.retryCount
      ) {
        ...
        log(
          ...
          'Error ' +
            ...
            (options.retries
              ? ' (' + options.retries - jobInfo.retryCount + ' retries left)'
              : '')
        );

        return error.options.retryTimeout;
      }
    });

    this.limiters.on('idle', async () => {
      log('debug', 'Queue size: %d', this.limiters.queued());
  ...
  async _schedule(options) {
    this.emit('schedule', options);
    try {
      await this.limiters.schedule(options, async () => {
        ...
        if (options.html) {
          ...
          options.uri(async (uri) => {
            options.uri = uri;
            await this._buildHTTPRequest(options);
          });
        } else {
          await this._buildHTTPRequest(options);
        }
      ...
      this._onContent(error.originalError, error.options);
    }
  }

  async _buildHTTPRequest(options) {
    ...
    const doRequest = async () => {
      ...
      try {
        ...
        throw new CrawlError(e, options);
      }

      this._onContent(null, options, response);
    };
