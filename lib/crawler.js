...
class Crawler extends EventEmitter {
  ...
  direct(options) {
    ...
    self.globalOnlyOptions.forEach(
  ...
  _scheduler(options) {
    ...
    self.limiters
      ...
      .submit(options.priority, function (done, limiter) {
  ...
  _buildHTTPRequest(options) {
    ...
    const doRequest = function (err) {
      ...
      request(_.pick.apply(self, [ropts].concat(requestArgs)), function (
        error,
        response
      ) {
        if (error) {
  ...
  _doEncoding(options, response) {
    ...
    if (options.forceUTF8) {
      const charset = options.incomingEncoding || self._parseCharset(response);
